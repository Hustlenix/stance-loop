...
import GhostStorageControls from "./GhostStorageControls";
import { hasGhostSession, loadGhostSession } from "./ghostSessions";
...
function SessionModal({ session, onClose, onTrain, onReplay, onTrainAgainstPast, pastSession, speakRecap }: { session: Session; onClose: () => void; onTrain: (drillId: DrillId) => void; onReplay: (sessionId: string) => void; onTrainAgainstPast: (sessionId: string) => void; pastSession?: Session; speakRecap?: boolean }) {
  ...
  const ghostAvailable = hasGhostSession(session.id);
...
function App() {
  ...
  const [ghostRaceId, setGhostRaceId] = useState<string | null>(null);
  const [raceResultSourceId, setRaceResultSourceId] = useState<string | null>(null);
  ...
  const completeSession = (session: Session) => {
    const completedRaceSource = ghostRaceId;
    saveSession(session);
    ...
    setLiveDrill(null);
    setGhostRaceId(null);
    setRaceResultSourceId(completedRaceSource);
  ...
  const trainAgainstPast = (sessionId: string) => {
    const ghost = loadGhostSession(sessionId);
    if (!ghost) return;
    setSelectedSession(null);
    setReplaySessionId(null);
    setRaceResultSourceId(null);
    setGhostRaceId(sessionId);
    setLiveDrill(ghost.drillId);
  };
  ...
  if (liveDrill) return <LiveCoach key={`${liveDrill}:${ghostRaceId ?? "solo"}`} drillId={liveDrill} ghostSessionId={ghostRaceId ?? undefined} preferences={preferences} onExit={() => { setLiveDrill(null); setGhostRaceId(null); }} onComplete={completeSession} level={level} onPreferencesChange={(next) => { savePreferences(next); setPreferences(next); }} onStepComplete={(session) => { saveSession(session); setSessions((previous) => [session, ...previous]); }} />;
  ...
  return <div className="app-shell">
    ...
    {selectedSession && <SessionModal session={selectedSession} onClose={() => { setSelectedSession(null); setLastFinishedId(null); }} onTrain={openDrill} onReplay={(id) => { setSelectedSession(null); setReplaySessionId(id); }} onTrainAgainstPast={trainAgainstPast} pastSession={raceResultSourceId ? sessions.find((entry) => entry.id === raceResultSourceId) : undefined} speakRecap={selectedSession.id === lastFinishedId} />}
    {replaySessionId && <ReplayLab sessionId={replaySessionId} onClose={() => setReplaySessionId(null)} onTrainAgainstPast={trainAgainstPast} />}
    {demoOpen && <ARDemo onClose={() => setDemoOpen(false)} />}
